...
describe('ComposableToken Test Suite', () => {
  ...
  const owner = 'STCYMDQ2B7WPTN1NDQ1HAADCYS1RKWGQ5KF09E61';
  ...
  describe('Deploying an instance of the contract', () => {
    before(async () => {
      await instance.mintToken(alice, 4, { sender: owner });
      await instance.mintToken(alice, 5, { sender: owner });
    });

    it("should initialize Alice's balance (2 asset)", async () => {
      const balanceAlice = await instance.balanceOf(alice);
      expect(balanceAlice).equal(4);
    });
    ...
    it('should make Alice owner of asset #1', async () => {
      const owner10001 = await instance.ownerOf(1);
      expect(owner10001).equal(`(ok (some ${alice}))`);
    });

    it('should make Alice owner of asset #2', async () => {
      const owner10002 = await instance.ownerOf(2);
      expect(owner10002).equal(`(ok (some ${alice}))`);
    });

    it('should make Bib owner of asset #3', async () => {
      const owner10003 = await instance.ownerOf(3);
